...
Driver.prototype.findLastId = function(query, callback){
	var result = [];
  this.Model.findAll({order: [[query, 'DESC']], limit : 1}).then(function(data){
     for (key in data){
      jsonString = JSON.stringify(data[key]);
      result[key] = JSON.parse(jsonString);
    }
...
Driver.prototype.update = function(query, update, callback){
  ...
  this.Model.find({where:query}).then(function(data, err){
    ...
    if(err){
      ...
      console.log(update);
      data.update(update).then(function(data, err){
        if(err){
...
Driver.prototype.delete = function(query, callback){
  this.Model.destroy({where:query}).then(function(data, err){
    if(err){
